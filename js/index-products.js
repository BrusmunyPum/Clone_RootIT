(function() {
  ...
  function renderProductCard(product, badgeLabel) {
    var badgeMarkup = badgeLabel
      ? '<div class="pro_best_selling"><span>' + safeValue(badgeLabel) + '</span></div>'
      : '';
    ...
    return [
      ...
      '        <img src="' + safeValue(product.image) + '" width="230" height="210" alt="' + safeValue(product.alt || product.title) + '" loading="lazy" class="img-responsive center-block">',
      '      </a>',
      '    </div>',
      '    <div class="pro_title">',
      '      <h3><a href="' + safeValue(product.href) + '">' + safeValue(product.title) + '</a></h3>',
      '    </div>',
  ...
  function renderProductSection(section) {
    var viewAllMarkup = section.viewAll
      ? '<a href="' + safeValue(section.viewAll) + '" class="btn-view-all">View All <i class="fa-solid fa-arrow-right"></i></a>'
      : '';
    var wrapperStyle = section.isMuted ? ' style="background-color: #f8f9fa;"' : '';
  ...
  function renderIndexProductSections() {
    ...
    mountNode.innerHTML = data.productSections.map(renderProductSection).join('');

    if (window.RootITAssets && typeof window.RootITAssets.initLocalAssets === 'function') {
